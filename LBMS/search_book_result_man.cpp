...
void search_book_result_man::slot2() {
	...
	if (ui.tableWidget->currentItem() != NULL) {
		currentRow = ui.tableWidget->currentItem()->row();
	}
	if (currentRow == -1) {
		QMessageBox::warning(NULL, QString::fromLocal8Bit("出错了"), QString::fromLocal8Bit("您未选择任何图书！"), QMessageBox::Ok);
	}
	else {
		...
		isbn = ui.tableWidget->item(currentRow, 0)->text();
		title = ui.tableWidget->item(currentRow, 1)->text();
		writer = ui.tableWidget->item(currentRow, 2)->text();
		publisher = ui.tableWidget->item(currentRow, 3)->text();
		many = ui.tableWidget->item(currentRow, 4)->text();
		inside = ui.tableWidget->item(currentRow, 5)->text();
		outside = ui.tableWidget->item(currentRow, 6)->text();
		ID = ui.tableWidget->item(currentRow, 7)->text();
